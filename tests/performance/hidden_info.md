...
The REFERENCES_CLAUSE edge in Graphiti (item 17) is the architectural keystone most people miss. Without it, your Neo4j graph and Postgres vector store are two separate silos that the LangGraph router visits independently. With it, a single Cypher traversal can return postgres_chunk_id values directly, which your Postgres hybrid retriever then uses as a WHERE chunk_id = ANY(:ids) hard filter — turning graph reasoning into a precision pre-filter for vector search rather than a parallel alternative. This is the difference between "we have a knowledge graph" (marketing) and "the graph surgically reduces the vector search space" (engineering).
The second thing nobody documents: docling's table extraction outputs markdown tables, but your tsvector indexing pipeline will tokenize the markdown syntax (|, ---) as noise tokens. Before writing to the BM25 column, strip markdown table syntax and re-serialize table content as natural language rows: "Row 3: Penalty Amount: $50,000; Trigger: Late delivery by more than 5 days". BM25 on raw markdown is nearly useless; BM25 on naturalized table prose is extremely powerful for legal schedule lookups.
