...
def _get_httpx_client_from_config(config: RunnableConfig | None) -> httpx.AsyncClient | None:
    ...
    http_client = configurable.get("httpx_client")
...
async def summarize_webpage(model: BaseChatModel, webpage_content: str) -> str:
    ...
    try:
        ...
        logger.bind(error=str(e)).warning("Summarization failed, returning original content")
        return webpage_content
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(token_url, headers=headers, data=form_data) as response:
                if response.status == 200:
                    ...
                    logger.bind(response_text=response_text).error("Token exchange failed")

    except Exception as e:
        logger.bind(error=str(e)).error("Error during token exchange")
