...
_RESEARCH_SYSTEM_PROMPT = render_prompt_sections(
    ...
    (
        "EXECUTION POLICY",
        "Start with broader searches, then narrow only to fill important gaps. Use think_tool after each search to assess results and plan next steps. Do not call think_tool in parallel with tavily_search or any other tool. Use Tavily for specific URLs and bounded source content. Stop when you can answer confidently rather than searching for perfection.",
    ),
    (
        "CONSTRAINTS",
