...
if TYPE_CHECKING:
    ...
    from app.features.billing.subscriptions.model import Subscription as SubscriptionModel
...
class DunningService:
    ...
    async def find_due_for_retry(self, *, limit: int = 200) -> DunningResult[list[Subscription]]:
        try:
            ...
            logger.bind(operation="dunning", table="subscriptions", error=str(exc)).exception(
                "find_due_for_retry failed"
            )
            return Failure(
    ...
    async def _attempt_charge(self, subscription: Subscription) -> DunningResult[dict[str, Any]]:
        ...
        try:
            ...
            if isinstance(plan_result, Failure):
                ...
                return Failure(
                    inner_value=DunningInfrastructureError(
            ...
            logger.bind(
                operation="dunning", subscription_id=str(subscription.id), error=str(exc)
            ).exception("Charge attempt failed")
            return Failure(
