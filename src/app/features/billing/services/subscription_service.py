...
from app.features.billing.services.proration_service import calculate_plan_change_proration
from app.utils import APIResponse, ValidationException, logger
...
if TYPE_CHECKING:
    ...
    from app.features.billing.models import Plan
...
class SubscriptionService:
    ...
    async def _get_owned_subscription(
        ...
        result = await self.repos.subscriptions.find_by_id(subscription_id)
    ...
    async def _refund_on_cancel(self, subscription: Subscription, plan: Plan, user_id: str) -> int:
        ...
        if not self._razorpay_enabled() or plan.refund_policy == "NONE":
            return 0
        result = await self.repos.payments.find_by_subscription(subscription.id, limit=1)
        ...
        payments = PaymentService(self.repos)
        payments.razorpay = self.razorpay
        ...
        if isinstance(result, APIResponse):
            logger.bind(operation="cancel_subscription").warning(result.error)
            return 0
        return refund_paisa
    ...
    async def create_subscription(
        ...
        existing = await self.repos.subscriptions.find_by_user_and_plan(user_id, plan.id)
        if isinstance(existing, Failure):
            return failure_envelope(existing.failure(), operation="create_subscription")
        ...
        result = await self.repos.subscriptions.create(subscription)
        if isinstance(result, Failure):
            return failure_envelope(result.failure(), operation="create_subscription")
        ...
        if self._razorpay_enabled():
            try:  # noqa: PLW0717
                ...
                update = await self.repos.subscriptions.update_with_lock(
                    created,
                    created.version,
                    values={
                ...
                if isinstance(update, Failure):
                    return failure_envelope(update.failure(), operation="create_subscription")
    ...
    async def list_subscriptions(
        ...
        *,
        ...
        limit: int = 50,
        offset: int = 0,
    ) -> ServiceResult[SubscriptionListResponse]:
        result = await self.repos.subscriptions.list_by_user(
            user_id, status=status, plan_id=plan_id, limit=limit, offset=offset
        )
        if isinstance(result, Failure):
            return failure_envelope(result.failure(), operation="list_subscriptions")
    ...
    async def get_subscription(
        ...
        result = await self.repos.subscriptions.find_by_id(subscription_id)
        if isinstance(result, Failure):
            return failure_envelope(result.failure(), operation="get_subscription")
        subscription = result.unwrap()
        if subscription is None:
            msg = "Subscription not found"
            raise ValidationException(msg)
    ...
    async def cancel_subscription(
        ...
        if dto.cancel_at_period_end:
            ...
            if isinstance(update, Failure):
                return failure_envelope(update.failure(), operation="cancel_subscription")
    ...
    async def pause_subscription(
        ...
        if isinstance(update, Failure):
            return failure_envelope(update.failure(), operation="pause_subscription")
        subscription = update.unwrap()
    ...
    async def resume_subscription(
        ...
        if isinstance(update, Failure):
            return failure_envelope(update.failure(), operation="resume_subscription")
        subscription = update.unwrap()
    ...
    async def change_plan(
        ...
        if isinstance(update, Failure):
            return failure_envelope(update.failure(), operation="change_plan")
        subscription = update.unwrap()
    ...
    ) -> ServiceResult[ProrationCalculation]:
        subscription = await self._get_owned_subscription(user_id, subscription_id)
        current_plan = await self._load_plan(subscription.plan_id)
    ...
    async def request_trial_extension(
        ...
        self.repos.session.add(extension)
