...
class PlanRepository:
    ...
    async def create(self, plan: Plan) -> PlanResult[Plan]:
        try:
            ...
            logger.bind(operation="create", plan_name=plan.name, error=str(exc)).exception(
                "plan_repository_failed"
            )
            return Failure(
                PlanConflictError(
            ...
            add_database_error_note(exc, table="plans")
            await self.session.rollback()
            logger.bind(operation="create", error=str(exc)).exception("plan_repository_failed")
            return Failure(
    ...
    async def find_by_id(self, plan_id: str | UUID) -> PlanResult[Plan | None]:
        try:
            ...
            add_database_error_note(exc, table="plans")
            await self.session.rollback()
            logger.bind(operation="find_by_id", plan_id=str(plan_id), error=str(exc)).exception(
                "plan_repository_failed"
            )
            return Failure(
                PlanInfrastructureError(
    ...
    async def find_by_name(self, name: str) -> PlanResult[Plan | None]:
        ...
        try:
            ...
            add_database_error_note(exc, table="plans")
            await self.session.rollback()
            logger.bind(operation="find_by_name", plan_name=name, error=str(exc)).exception(
                "plan_repository_failed"
            )
            return Failure(
                PlanInfrastructureError(
    ...
    async def list_active(self, *, limit: int = 50, offset: int = 0) -> PlanResult[list[Plan]]:
        try:
            ...
            add_database_error_note(exc, table="plans")
            await self.session.rollback()
            logger.bind(operation="list_active", error=str(exc)).exception("plan_repository_failed")
            return Failure(
                PlanInfrastructureError(
    ...
    async def archive(self, plan_id: str | UUID) -> PlanResult[Plan | None]:
        ...
        try:
            ...
            add_database_error_note(exc, table="plans")
            await self.session.rollback()
            logger.bind(operation="archive", plan_id=str(plan_id), error=str(exc)).exception(
                "plan_repository_failed"
            )
            return Failure(
                PlanInfrastructureError(
    ...
    async def update(self, plan: Plan, *, values: dict[str, object]) -> PlanResult[Plan]:
        try:
            ...
            add_database_error_note(exc, table="plans")
            await self.session.rollback()
            logger.bind(operation="update", plan_id=str(plan.id), error=str(exc)).exception(
                "plan_repository_failed"
            )
            return Failure(
                PlanConflictError(
