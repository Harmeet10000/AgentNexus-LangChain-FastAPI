...
from langchain_layer.models import aembed_batch, ainvoke_multimodal
...
async def batch_invoke(agent_name: str, request: AgentBatchRequest):
    ...
    if len(request.messages) != len(request.thread_ids):
        raise HTTPException(
            status_code=422, detail="messages and thread_ids must have the same length"
        )
    ...
    responses = [
        _extract_response(s, tid) for s, tid in zip(states, request.thread_ids)
    ]
