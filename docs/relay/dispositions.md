...
| 195 | 282 | postgres vector + BM25 + RRF, langextract **before** these, hybrid + re-ranking | **IN 1, reshaped** | Not greenfield (D5.1): BM25 (`search/repository.py:415-419`) and RRF (`fusion.py:28`, k=60) exist. Work = harvest those into the unified path + **add re-ranking** (genuinely missing) + move langextract upstream (D9). |
...
| 176 | 270 | check `sentence_transformers` / `AutoTokenizer`, or a langchain replacement | **IN 1, small** | Scoped to: if the only use is token counting, drop the direct `transformers` dependency. Verification = import no longer present. |
...
| Up#10 | 432-440 | tool output normalization — one `ToolResult` | **IN 3** | Three competing definitions: `langchain_layer/agents/tools/idempotency.py:34` (survivor), `shared/agents/tools/idempotency.py:11`, `rag/document_processing/models.py:318`. Direct extension of D6.1. |
