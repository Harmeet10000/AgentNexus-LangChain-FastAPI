...
- [x] 1.2 Record the migration head verbatim from the `alembic current` output that `rag-tree-repair`
  produced. **Do not guess a head.**
  **Proof:** the recorded value equals a `revision` literal present under `src/alembic/versions/`.
- [x] 1.3 Record whether the `chunks` table is populated.
...
- [x] 1.4 Record the tier-1 retrieval baseline produced by `rag-eval-harness`.
  **Proof:** `baseline.md` names the golden-set version and the commit identifier of the report it
  cites.

## 2 Chunk identity

- [x] 2.1 Add `document_version: int` and `locus: str | None` to `UnifiedChunk`
...
- [x] 2.2 One migration, with `down_revision` set from the value recorded in 1.2.
  **Proof:** `uv run alembic check` reports no pending autogenerate diff.
- [x] 2.3 The write path populates both fields.
...
- [x] 3.1 Pure `resolve_chunk_policy(document_kind) -> ChunkPolicy` in
...
- [x] 3.2 Call it from the docling path and record the resolved policy on every chunk.
  **Proof:** a fixture ingestion asserts chunk metadata carries the policy name.

## 4 Clause post-pass

- [x] 4.1 Pure `recover_clause_numbering(chunks)` reusing `_CLAUSE_START_RE`
...
- [x] 4.2 Wire it after `_hybrid_chunk_documents`.
  **Proof:** a fixture ingestion populates `locus`.

## 5 The contextualize split

- [x] 5.1 **Settle the token-budget question first.** Write a test asserting that for every
...
- [x] 5.2 Change `chunker.py:254` from `content=contextualized_text.strip()` to `content=chunk.text`
...
- [x] 5.3 Flag fallback chunks impure; set overlap to zero on every non-fallback policy.
...
- [x] 6.0 **Read before editing.** `unified-embedding`'s requirement *"Every embedding consumer resolves
...
- [x] 6.1 Repoint `src/app/shared/rag/docling/ingest_v2.py:18` at `embed_texts`; delete
  `_PROVIDER_EMBEDDING_MODEL` from `docling/embedder.py`.
  **Proof:** `rg -c 'gemini-embedding-001' src/` returns `0`.
- [x] 6.2 Update `tests/unit/shared/rag/test_embedder_no_substitution.py` and
  `test_rag_agent_embedder_import.py`.
  **Proof:** pytest summary pass count ≥ the 1.1 baseline.

## 7 Dependency drop

- [x] 7.1 Swap `docling_enhanced.py:63-71`'s `PdfPipelineOptions` to RapidOCR. Audit the two bare
...
- [x] 7.2 Drop `sentence-transformers` (`pyproject.toml:51`) and `langchain-docling` (`:43`).
...
- [x] 7.3 The token-counter cache guard still holds.
  **Proof:** `tests/unit/shared/rag/test_chunker_tokenizer_cache.py` is green.

## 8 Layering

- [x] 8.1 Add a public `build_chat_model` factory; repoint `documents/dependencies.py:13`,
  `documents/service.py:19,26`, and `crawler/processor.py:14`.
  **Proof:** the `PLC2701` count goes from 3 to 0 against the 1.1 baseline.
- [x] 8.2 Retire the five callers of `features/documents/chunking.py::chunk_text` onto the docling path;
...
- [x] 9.1 **Proof:** `openspec validate ingestion-chunking --strict` exits `0`;
...
- [x] 9.2 Re-verify the named blast radius: `test_chunker_tokenizer_cache.py`,
