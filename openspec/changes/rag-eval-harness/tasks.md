...
- [x] 1.1 `src/app/shared/evaluation/metrics.py` — `recall_at_k`, `reciprocal_rank`, `ndcg_at_k`,
...
- [x] 1.2 Assert the module is genuinely pure — no import of a database, HTTP, or model-provider
  symbol.
  **Proof:** `rg -n "sqlalchemy|httpx|langchain|asyncpg|redis" src/app/shared/evaluation/metrics.py; test $? -eq 1`
  → exit `0`.
- [x] 1.3 Property tests using the existing `hypothesis` dependency and `property` marker: recall@k is
...
- [x] 2.1 `src/app/shared/evaluation/schema.py` — Pydantic v2 `GoldenQuery` (query text, expected chunk
  ...
  **Proof:** `uv run pytest tests/unit/shared/evaluation/test_golden_schema.py -q` summary shows zero
  failures.
- [x] 2.2 Loader returning a `Result` failure for a malformed row and raising a typed project exception
...
- [x] 2.3 Seed corpus `evals/golden/legal_retrieval_v1.jsonl` — deliberately small, covering contracts,
...
- [x] 3.1 `src/app/shared/evaluation/runner.py` — `run_retrieval_eval(*, queries, retrieve)` where
  ...
  **Proof:** a unit test with a fake retriever produces aggregates equal to hand-computed values; and
  ...
  `0`.
- [x] 3.2 `report.py` — JSON carrying commit identifier, timestamp, golden-set version, per-query rows,
  and aggregates.
  **Proof:** a round-trip unit test passes, and a second asserts the report names the golden-set
  version it scored.
- [x] 3.3 Binding entrypoint reaching retrieval through the documents **service**, never the
...
- [x] 3.4 **Liveness probe — the harness must be proven to reach real retrieval.** Every Proof above
...
- [x] 4.1 Produce the first report against the seeded live path from 3.4. "No environment available"
...
- [x] 4.2 Confirm the default gate is unchanged by this change.
...
- [x] 5.1 Define the report's judged section as an optional, absent-by-default block, and assert that
...
- [x] 5.2 Record in `review.md` what the later judged layer will add and what it must not change:
...
- [x] 6.1 **Proof:** `openspec validate rag-eval-harness --strict` exits `0`; ruff and ty counts ≤ their
