...
- [x] 1.1 Capture `uv run pytest tests/unit/shared/langgraph_layer -q` counts into
  ...
  **Proof:** the file lists `test_reranker_singleton.py` and `test_retrieval_retry_shape.py` with their
  current outcomes.
- [x] 1.2 **Pin the graph's current shape as a test.** Record `build_retrieval_graph`'s node names and
...
- [x] 1.3 Record the tier-1 retrieval baseline from `rag-eval-harness`.
...
- [x] 2.1 Define a `Reranker` protocol matching the call already made at `nodes.py:263`
...
- [x] 2.2 Add a hosted implementation behind the protocol, selected by settings, with the local one
  still available.
  **Proof:** a test injecting a stub hosted client asserts twenty candidates in, `limit` out, in the
  provider's returned order.
- [x] 2.3 Degradation test: the provider raises, the node returns the fused order truncated to `limit`,
  and no exception escapes.
  **Proof:** `uv run pytest tests/unit/shared/langgraph_layer -q` gains one test over the 1.1 baseline.
- [x] 2.4 Make the hosted implementation the configured default. Record in `design.md` that deleting
...
- [x] 3.1 Extend `QueryPlan` with an allowlist field, and add a `source_identifier` node between
...
- [x] 3.2 **Widen the allowlist on retry.** When the grader reports insufficient context, the next
...
- [x] 3.3 Assert the widening cannot extend the loop.
...
- [x] 4.1 Insert a `post_process` node between the reranker and the context grader that dedupes by chunk
...
- [x] 4.2 Replace `len(content.split())` at `rag.py:79,87` with the token counter, **injected as a
...
- [x] 4.3 Record in `design.md` that the tokenizer **choice** belongs to `ingestion-chunking`, and that
...
- [x] 5.1 **Proof:** `openspec validate agentic-retrieval --strict` exits `0`;
  ...
  here.
- [x] 5.2 Re-verify the named blast radius:
...
- [x] 5.3 Re-run the tier-1 retrieval eval and record the delta against the 1.3 baseline, **whether it
