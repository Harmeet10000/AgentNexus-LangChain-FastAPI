...
- **THEN** `cognee.improve(datasets=[dataset_name])` is called
...
The system SHALL use `cognee.recall()` to query stored memories, replacing the deprecated `cognee.search()`. The `SearchType.INSIGHTS` enum SHALL continue to be used as the search mode parameter.

#### Scenario: Search episodic memory
- **WHEN** `search_episodic_memory()` is called with a `query` and `user_id`
- **THEN** `cognee.recall(SearchType.INSIGHTS, query=query, datasets=[dataset_name])` is called
