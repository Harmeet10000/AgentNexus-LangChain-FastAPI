...
- [x] 11.1 Record the hard ordering constraint in the next change's proposal: `shared/services/` must land **before `crawler`**, because `crawler/service.py:18` imports `search` — re-exported from `tavily.py`, which raises 8 exceptions. Not because of `rate_limiter.py`, which raises nothing and catches nothing
  > **DONE (carried by §13):** ordering recorded and satisfied in-change — task 13.5 re-verified the edge (`features/crawler/service.py:20` imports `search` from `shared/services`, re-exported from `tavily.py`) and gated `crawler` on 13.1–13.3, which landed first. `rg` on the module name cannot see this edge; it is a symbol import through a package `__init__`.
- [x] 11.2 Record that Phase 1a covers three modules, not four: `storage.py` (21 raises), `tavily.py` (8), `mailer.py` (2, no importer outside the package so it blocks nothing); `rate_limiter.py` is excluded
  > **DONE (carried by §13):** scope landed as tasks 13.1 (storage union), 13.2 (tavily union), 13.3 (mailer union); 13.4 re-verified `rate_limiter.py` has zero `raise`/zero `except` and stays excluded with no union introduced.
- [x] 11.3 Record that 4 of `tavily.py`'s 8 raises are pre-flight argument guards rather than third-party classification, and that 17 of `storage.py`'s 21 are `ServiceUnavailableException` which keeps its 503 — so that conversion has no observable break
  > **DONE (carried by §13):** 13.2 keeps the four pre-flight guards as `TavilyValidationError` and classifies only the other four as external-service; 13.1 records that 17 of storage's 21 keep 503 with no observable status break. Verified in `shared/services/errors.py` (`StorageResult`/`TavilyResult`).
- [x] 11.4 Confirm the feature order and its rationale: `search` → `audit` → `crawler` → `users` → `ingestion` → `dunning` → `profile` → `plans` → `invoices` → `payments` → `webhooks` → `agent_saul` → `health` → `credits` → `documents` → `auth`
  > **DONE (superseded by §12 — substance holds in corrected form):** this 16-entry order is stale: `search` is a tombstone with no code to convert and `health` is classify-only (converting it would override its probe-as-data 200/503 contract — a regression, not progress). The corrected 14-entry order `audit` → `crawler` → … → `auth` is published in `proposal.md:167`, `design.md:682`, and `HANDOFF.md:386` via tasks 12.2–12.4, and all 14 conversions landed under §15.
- [x] 11.5 Record that **18** features exist, not 17: `subscriptions` migrates here as the exemplar and `chat` needs no change at all (`__init__.py` and `model.py`, zero raises, zero `except` clauses). Phase 2 is therefore 16 changes — 18 = 1 + 16 + 1 — and two deferred changes follow: `shared/crawler/` alongside `crawler`, and `shared/rag/`'s provider boundary alongside `documents`. `utils/cache/` is **not** deferred; it is task 7.5 here
  > **DONE (superseded by §12 — substance holds in corrected form):** the arithmetic is corrected to **18 = 1 exemplar + 14 conversions + 2 no-ops + 1 classify-only** (task 12.2, recorded in `proposal.md:172`, `design.md:603`, `HANDOFF.md:386`): `chat` and tombstoned `search` are no-ops, `health` is classify-only (12.3). Both deferred boundaries landed in-change (§14: `shared/crawler/` with `crawler`, `shared/rag/` provider boundary with `documents`); `utils/cache/` stays non-deferred per 7.5.
- [x] 11.6 Carry the per-feature exit criteria into each feature change's tasks as its own checklist
  > **DONE (carried by §15 + §17):** the §15 header requires every conversion to carry the per-feature exit criteria, each 15.x task records its feature-specific surface/hazard with a DONE block, and §17 gates the whole program on the measurable zeros (contracts, unions, fixtures, exclusions, dual enumeration). Verified live: 15 `errors.py` contracts, zero `AppError` sites, zero cross-feature error imports.
- [x] 11.7 Carry all three Method notes into each feature change's review step: (1) enumerate a population by a second, structurally different query before a count becomes a claim; (2) `ls` the paths a plan says it will create and match the probe to the edge kind — `rg` cannot see symbol imports, `python -c "import x"` cannot see `TYPE_CHECKING` ones; (3) before citing a gate's zero, read its exclusion list
  > **DONE (carried by §15 header + §17 + review):** the §15 header binds all three Method notes to every conversion review; 17.6 re-reads every gate's fixture pair and exclusion list (`per-file-ignores`, `sgconfig.yml` ruleDirs, rule-level filters); 17.7 derives totals twice by structurally different queries and greps DONE blocks for partial/deferred/TODO. `review.md` Method notes and `HANDOFF.md` §6–§7 keep the rules.
