...
- [ ] 2.1 Insert a real `## Purpose` header into `openspec/specs/cognee-v1-api/spec.md` as a **one-line direct file
...
- [ ] 2.2 Capture the pre-code validation baseline so any later regression is attributable.
...
- [ ] 3.1 Add an `include_object` / `include_name` filter to `src/alembic/env.py` that excludes the memory schema
...
- [ ] 3.2 Wire the filter into **both** `context.configure(...)` calls, not just the online one.
...
- [ ] 4.1 Add the `COGNEE_*` settings surface, which does not exist today (`rg -i cognee src/app/config/settings.py`
...
- [ ] 4.2 Write the access-control setting into the process environment **explicitly, before the first memory
...
- [ ] 4.3 Pin the embedding model to the repository's configured embedder and derive the dimension from
...
- [ ] 4.4 Configure the vector store explicitly so the library default is never reached — `set_vector_db_config` is
...
- [ ] 4.5 Make the relational configuration resolve to the application's own database: derive the discrete fields
...
- [ ] 4.6 Rename the shadowed local `config` at `cognee_client.py:107` and stop returning `postgres_url` from it —
...
- [ ] 4.7 Replace the `dict[str, Any]` startup return with a typed configuration result, so the health probe has
...
- [ ] 4.8 Guard the startup call. `lifespan.py:206` `await setup_cognee(settings)` is today the **only** unguarded
...
- [ ] 5.1 Add `check_cognee` to `src/app/middleware/health_check.py` and register it in `ALL_PROBES` (`:93-99`, where
...
- [ ] 5.2 Add the same probe to the second surface, `src/app/features/health/service.py`, under a field name that
...
- [ ] 5.3 Report the graph-procedure precondition as a **named sub-field**, and do not fail the whole check when it is
...
- [ ] 6.1 Replace the three bare partition-name interpolations (`cognee_client.py:140,189,238`) with **one validated
...
- [ ] 6.2 Implement the conversation-scoped report write: a conversation identity is required, and self-improvement is
...
- [ ] 6.3 Implement the typed trace / QA / feedback writes, and reject a write with no conversation identity **before
      ...
      **Proof:** a test asserting the rejection raises the caller-error type **and** that the faked memory module was
      never called at all.
- [ ] 6.4 Implement recall with a **full model dump** and the origin field preserved — the existing
...
- [ ] 6.5 Implement consolidation as the **only** caller of enrichment, and report what it consolidated (conversation
...
- [ ] 6.6 Make consolidation abort with a **named precondition failure** when the required graph procedures are
...
- [ ] 6.7 Settle on one failure idiom across the service, keeping `e.add_note()` before re-raise (house style, live at
...
- [ ] 6.8 Guard against the prune operation ever being called — the permanent memory graph and the document entity
...
- [ ] 7.1 Retarget the memory-persist node onto the service, gated on human approval.
...
- [ ] 7.2 Preserve the fail-open shape: a memory failure records `COGNEE_WRITE_FAILED` (already present at
...
- [ ] 7.3 Stop writing the final report to the document knowledge graph — the accepted boundary expressed in code.
...
- [ ] 8.1 Harvest the two helpers that have **no other implementation in the repository** into
...
- [ ] 8.2 Add the prefetch node after clarification: agent memory queried first, with a **bounded** knowledge-graph
...
- [ ] 8.3 Keep the read path fail-open (the pattern already at `memory_pipeline.py:258-260`).
...
- [ ] 8.4 Add the deeper-retrieval **service operation** with its role restriction: available to the risk-analysis and
...
- [ ] 8.5 Do not make the agent graph reachable, and do not let this seam make re-enabling it harder (D17, NG10).
...
- [ ] 9.1 **Depends on change 0.** Confirm the reconciliation re-exports are gone from
...
- [ ] 9.2 Add the consolidation task with a real `@celery_app.task` decorator (the reconciliation module never had
...
- [ ] 9.3 Add one beat entry on a nightly schedule, named distinctly from the existing billing reconciliation entry —
...
- [ ] 10.1 Delete, in **one commit with their paired re-export edits**: `write_final_report.py`,
      ...
      **Proof:** `uv run python -c "import app.main"` succeeds — this is the load-bearing proof for the whole group,
      ...
      about a deletion.
- [ ] 10.2 Delete caller and callee together. `write_final_report.py:122,146` call the memory functions through the
      ...
      **0 hits**.
- [ ] 10.3 Confirm no count regressed across the deletions.
      **Proof:** `uv run ruff check src/ 2>&1 | tail -1` ≤ **123**; `uv run ty check src/ 2>&1 | tail -1` ≤ **46**;
      ...
      green run exit 1).
- [ ] 10.4 **Archive** `openspec/changes/cognee-saul-memory-migration` — **never delete it.** Its `proposal.md:20-21`
...
- [ ] 10.5 Re-validate, and confirm the archive of the superseded change did not add a seventh failure — the new
