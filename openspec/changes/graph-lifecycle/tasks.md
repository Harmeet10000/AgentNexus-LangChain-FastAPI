...
- [x] 1.1 `src/app/lifecycle/graphs.py` — provider functions only, with no I/O performed at build time.
...
- [x] 1.2 Checkpointer startup policy calling
...
- [x] 1.3 **Landmine check.** That test file includes proofs that grep `checkpointer.py` for *absent*
  ...
  **Proof:** `rg -n 'from_conn_string|hasattr\(.*"pool"' src/app/shared/langgraph_layer/checkpointer.py; test $? -eq 1`
  → exit `0`.
- [x] 1.4 Assert no credential reaches a log line on a setup failure.
...
- [x] 2.1 Construct the memory service inside the same provisioning path. Its constructor
...
- [x] 2.2 Saul graph policy assigning the graph to process state.
...
- [x] 2.3 Confirm memory semantics are not respecified here. Cognee owns them, and
...
- [x] 3.1 Resolve the repository from the invocation configuration inside the node, instead of from a
  ...
  **Proof:** `uv run pytest tests/unit/documents tests/unit/shared/langgraph_layer/test_ingestion_checkpoint_plumbing.py -q`
  ...
  missing key.
- [x] 3.2 Assert no compiled graph captures a session or repository.
...
- [x] 4.1 Point `run_document_ingestion_task` (`src/app/features/documents/service.py:904-947`) at a
  ...
  **Proof:** a unit test patches the builder with a counting spy and asserts one compilation across two
  invocations.
- [x] 4.2 **Highest-risk task in this cluster.** Hoist the per-task construction at
...
- [x] 4.3 Assert release is not attempted for a resource that was never constructed.
...
- [x] 4.4 Register both hooks, following the existing prior art at `src/tasks/crawler_tasks.py:72`.
...
- [x] 5.1 Register the ingestion graph as a startup policy; delete the commented block at
...
- [x] 5.2 Rewrite the two prose sites that forbid this wiring —
...
- [x] 5.3 **Write the upload-to-chunks acceptance check** that the archived rule forbade. It was
...
- [x] 6.1 `src/app/shared/langgraph_layer/open_deep_search/graph.py:278,478,555` — convert the three
  ...
  **Proof:** a unit test imports the module with a spy on the compile call and asserts zero invocations
  at import time.
- [x] 6.2 `app.state.pageindex_client` (`lifespan.py:538`) stays unwired. `knowledge-stack` owns its
...
- [x] 7.1 **Proof:** `openspec validate graph-lifecycle --strict` exits `0`;
  ...
  failure count ≤ `docs/relay/baseline-pytest.txt`.
- [x] 7.2 Confirm Agent Saul is actually on.
